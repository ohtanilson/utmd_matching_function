...
utmd_output_hello_work_data_pooling_part_and_full_time_monthly <-
  readRDS(file = here::here("output/utmd_output_hello_work_data_pooling_part_and_full_time_monthly.rds")) %>% 
  dplyr::rename(
    `unemployed`= candidate_count,
    `vacancies` = position_count,
    `hire` = hire_count,
...
### Month-level pooling full-time and part-time 
...
x
figure_name <-
  "../figuretable/residual_plot_month_pooling_part_and_full_time.png"
ggsave(filename = figure_name,
       plot = x,
       device = "png",
       width = 4,
...
x <- 
  ...
  #theme(axis.text.x = element_text(angle=60, vjust=1, hjust=1, size=3.0)) +
  xlab("") +
  ylab("A (normalized to 1972 Jan full-time)") +
  labs(colour = "") +
  #geom_vline(xintercept = 2002, linetype = "longdash") +
  # geom_vline(xintercept = 1984, linetype = "longdash") +
  scale_color_viridis_d()
...
# Appendix: Separate estimation of full-time and part-time {.tabset}

## Residual plot  {.tabset}
