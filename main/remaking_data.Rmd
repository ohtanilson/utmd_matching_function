---
title: "一般職業紹介状況データの作成について"
author: "Hayato Kanayama"
date: "2024-06-18"
...
データについては、厚生労働省の職業安定業務統計から引っ張ってきている
...
prefecture_data %>%
     write_rds(x = ., 
               file = "data_output/vacancy_unemployed_hire_prefectrue_non_job_type.rds")
...
data_prefecture <- data_prefecture %>%
...
## 数値化する変数のベクトル
var_vec <- names(data_prefecture)
var_vec <- setdiff(var_vec, c("year_month", "prefecture", "type", "contract_type"))

for(i in var_vec){
     eval(parse(text = paste0(
          "data_prefecture <- data_prefecture %>% mutate(", i," = as.numeric(", i,"))"
               )
          )
     )
}
...
## 変数をnumeric化
vec_name <- data_prefecture %>% names()
...
write_rds(
     data_prefecture,
     file = "data_output/vacancy_enemployed_hire_prefecture.rds"
)

## data_prefectureの結果を確認
data_prefecture_hokkaido <- data_prefecture %>%
        filter(
           prefecture == "北海道",
                 type == "both",
        contract_type == "both"
...
data_long_wide %>%
     write_rds(., file = "data_output/unemployed_gender.rds")
